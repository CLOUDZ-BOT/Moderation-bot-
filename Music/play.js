...
async execute(message, args, client) {
    ...
    try {
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#303037")
            .setDescription(`**<:s_searching:854543865053577228>   Searching  [LINK](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("#303037")
            .setDescription(`**<:s_searching:854543865053577228>   Searching  \`${args.join(" ")}\`**`))
        }
      } else {
        ...
        //send join message
        message.channel.send(new MessageEmbed().setColor("#303037")
          .setDescription(`**<:s_voice:854540365112541224> Joined \`${channel.name}\`\n\n<:s_channel:854566619099037696> bound \`${message.channel.name}\`**`)
    ...
    //get the thumbnail
    let thumb = "https://cdn.discordapp.com/emojis/854540645507137576.gif"
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        .setTitle("<a:s_playing:854540645507137576> " + song.title)
        .setColor("#303037")
        .setThumbnail(thumb)
        .setURL(song.url)
        .setDescription(`\`\`\`Has been added to the Queue.\`\`\``)
        .addField("<a:s_time:854540332544557056> Estimated time until playing:", `\`${estimatedtime}\``, true)
        .addField("<a:s_uptime:850689026100035584> Position in queue", `**\`${serverQueue.songs.length - 1}\`**`, true)
