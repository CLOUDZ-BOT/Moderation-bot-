...
module.exports = {
  async play(song, message, client, filters) {
    ...
    if (!song) {
      ...
      const endembed = new MessageEmbed().setColor("#303037")
       .setDescription("<a:s_ping:854540349426237441> Music Queue ended.\n\n<a:s_playing:854540645507137576> To play music type +play <Song/Link>")
  ...
  let thumb;
    if (song.thumbnail === undefined) thumb = "https://cdn.discordapp.com/emojis/854540645507137576.gif";
    else thumb = song.thumbnail.url;

  try {
      let embed = new MessageEmbed()
          .setDescription(`<a:s_playing:854540645507137576> Started Playing\n\n[**${song.title}**](${song.url})\n\n<a:s_time:854540332544557056> Duration : ${song.duration}`)
          .setThumbnail(song.thumbnail.url)
          .setColor("#303037")
          .setFooter(`Requested by: ${message.author.username}#${message.author.discriminator}`, message.member.user.displayAvatarURL({ dynamic: true }))
      var playingMessage = await queue.textChannel.send(embed);
      var playingMessage = await queue.textChannel.send(`\`\`\`SONG SETTINGS ⚙️\n\n⏭ Skips the current song\n⏯ Pauses/Resumes the current song\n🔉 Decreases the volume\n🔊 Increases the volume\n🔇 Mutes/Unmutes the Music\n🔁 Enables/Disables loop\n🔀 Shuffles the queue\n⏹ Stops the music\n🎵 Shows info about current song\n🎶 Shows
        the queue\n📑 Shows the lyrics of the song\`\`\``);
    ...
    collector.on("collect", async (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        let nowPlaying = new MessageEmbed()
          .setAuthor('Now playing','https://cdn.discordapp.com/emojis/854540645507137576.gif')
      ...
      if (ms > 0 && ms<10000) {
        nowPlaying.addField("\u200b", "**``[" + createBar((ms == 0 ? seek : ms), seek, 25, "▬", "🔘")[0] + "]``**\n**" + "\n[" + new Date(seek * 1000).toISOString().substr(11, 8) + " / " + (ms == 0 ? " ◉ LIVE" : new Date(ms * 1000).toISOString().substr(11, 8))+ "]**" + "\n" + "\n **Time Remaining:**" + "``" + new Date(left * 1000).toISOString().substr(11, 8) + "``", false );
          ...
          .setAuthor(" Searching...", "https://cdn.discordapp.com/emojis/854543865053577228.png").setFooter("Lyrics")
          .setColor("#303037")
    ...
    collector.on("end", () => {
      ...
      if (PRUNING && playingMessage && !playingMessage.deleted) {
        playingMessage.delete({ timeout: 300000 }).catch(console.error);
      }
    });
  }
};